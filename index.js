...
let contributions = GAMES_JSON.reduce((acc, conts) => {
...
let monies = GAMES_JSON.reduce((acc, conts) => {
    return acc + conts.pledged;
}, 0);

// set inner HTML using template literal
raisedCard.innerHTML = "$" + monies.toLocaleString('en-US');
...
function filterUnfundedOnly() {
    ...
    let unfundedgames = GAMES_JSON.filter( (agame) => {
...
function filterFundedOnly() {
    ...
    let fundedgames = GAMES_JSON.filter( (agame) => {
        return agame.pledged >= agame.goal;
    });
